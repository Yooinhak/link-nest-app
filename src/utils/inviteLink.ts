/**
 * 초대 딥링크 유틸.
 * MVP: 커스텀 스킴(moaring://invite/<token>).
 * 구 스킴(linkle://, link-nest-app://)도 app.json scheme 배열에 남겨 기존 공유 링크 호환 유지 —
 * 파서는 스킴 비의존이라 모두 처리된다.
...
export function buildInviteUrl(token: string): string {
  return `moaring://invite/${token}`;
}

export function buildInviteMessage(groupName: string, token: string): string {
  return `모아링 '${groupName}' 그룹에 초대해요!\n${buildInviteUrl(token)}`;
}

/**
 * URL에서 초대 토큰 추출. 다음 형태를 모두 지원 (스킴 비의존):
 * - moaring://invite/<token> · linkle://invite/<token> · link-nest-app://invite/<token> (구 스킴)
 * - moaring://invite?token=<token>
