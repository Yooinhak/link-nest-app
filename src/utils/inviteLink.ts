/**
 * 초대 딥링크 유틸.
 * MVP: 커스텀 스킴(link-nest-app://invite/<token>).
 * TODO: 시안 ⑨ 웹 랜딩 배포 후 https 유니버설 링크로 교체.
 *
...
export function buildInviteUrl(token: string): string {
  return `link-nest-app://invite/${token}`;
}

export function buildInviteMessage(groupName: string, token: string): string {
  return `Link Nest '${groupName}' 그룹에 초대해요!\n${buildInviteUrl(token)}`;
}

/**
 * URL에서 초대 토큰 추출. 다음 형태를 모두 지원:
 * - link-nest-app://invite/<token>
 * - link-nest-app://invite?token=<token>
